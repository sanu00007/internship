...
void type_mapping(FILE *fp,int type){
...
void print_number_tcs(double value, CurrencyFormat *currency_format, 
...
static void print_tcs_account(json_t* node,  CurrencyFormat *currency_format,
  const char *currency_symbol, vector<data_map *> *contact_mapping, FILE *fp,int date_format) {
  ...
  const char* third_part = "</span></td><td  style=\"text-align: left; \"class=\"mat-cell\"><span  class=\"font-weight-500\">";
  fwrite(third_part, 1, strlen(third_part), fp);

  int type;
  json_handle = json_object_get(node, "type");
  if(json_handle && json_is_integer(json_handle)){
    type = json_integer_value(json_handle);
  }
  type_mapping(fp, type);//function to map based on type number

  const char* fourth_part = "</span></td><td class=\"mat-cell\" style=\"top: 0px; z-index: 100; text-align:left\"><span  class=\"app-link icon-hover\">";
  fwrite(fourth_part, 1, strlen(fourth_part), fp);

  json_handle = json_object_get(node, "customer_id");
  if(json_handle && json_is_integer(json_handle)){
    int customer_id = json_integer_value(json_handle);
  ...
  double value;
  value=0;
  json_handle = json_object_get(node, "rate");
  if(json_handle && json_is_number(json_handle)){
    value=json_number_value(json_handle);
  }
  print_number_tcs(value, currency_format, currency_symbol, fp);
  ...
  const char* eigth_part = "</span></td><td class=\"mat-cell\" style=\"top: 0px; z-index: 100; text-align:left\">";
  fwrite(eigth_part, 1, strlen(eigth_part), fp);

  value=0;
  json_handle = json_object_get(node, "tcs_amount");
  if(json_handle && json_is_number(json_handle)){
    value=json_number_value(json_handle);
  }
  print_number_tcs(value, currency_format, currency_symbol, fp);

  const char* nineth_part = "</span></td><td class=\"mat-cell\" style=\"top: 0px; z-index: 100; text-align:left\">";
...
static void print_tcs_accnts_from_array(json_t* arr,  CurrencyFormat *currency_format, 
  ...
  json_array_foreach(arr, index, node){
    if(json_is_object(node)){
      print_tcs_account(node, currency_format, currency_symbol, contact_mapping, fp,date_format);
    }
  } 
}
...
bool process_tcs_summary(const json_t *root, int from_date,int to_date, int date_format, 
  ...
  do{
    ...
    const char* third_part =  "</h4></div><div class=\"report-holder wide-report-holder\"><table class=\"mat-table  mat-sort w-100\" role=\"table\" style=\"border-collapse: separate; border-spacing: 10px;\"><table  mat-table=\"\" matsort=\"\" class=\"mat-table  mat-sort w-100\"><thead \"> <tr mat-header-row=\"\" class=\"mat-header-row  ng-star-inserted\" style=\"\"> <th mat-header-cell=\"\" class=\"mat-header-cell\" style=\"height:50px; top: 0px; z-index: 100;\">Date</th><th mat-header-cell=\"\" class=\"mat-header-cell  \" style=\"top: 0px; z-index: 100;\">Transaction#</th><th  mat-header-cell=\"\" class=\"mat-header-cell\" style=\"top: 0px; z-index: 100;\">Type</th> <th mat-header-cell=\"\" class=\"mat-header-cell  p\" style=\"top: 0px; z-index: 100;\">Customer</th> <th mat-header-cell=\"\" class=\"mat-header-cell\" style=\"top: 0px; z-index: 100;\">PAN</th> <th style=\"text-align: left; mat-header-cell=\"\" class=\"mat-header-cell \" style=\"top: 0px; z-index: 100;\">Rate</th> <th  style=\"text-align: left; mat-header-cell=\"\" class=\"mat-header-cell\" style=\"top: 0px; z-index: 100;\">Total</th> <th style=\"text-align: left; mat-header-cell=\"\" class=\"mat-header-cell\" style=\"top: 0px; z-index: 100;\">TCS</th><th  style=\"text-align: left; mat-header-cell=\"\" class=\"mat-header-cell \" style=\"top: 0px; z-index: 100;\">Code</th></tr></thead>";
    fwrite(third_part, 1, strlen(third_part), fp);
    ...
    if(arr_handle && json_is_array(arr_handle)){
      print_tcs_accnts_from_array( arr_handle,currency_format, 
                            currency_symbol, &contact_mapping, fp,date_format);
    }

    const char* last_part = "</div>" ;
    fwrite(last_part, 1, strlen(last_part), fp);
    ret=true;
  }while(0);
