...
static double print_cf_accnts_from_array(json_t* arr, int level, CurrencyFormat *currency_format, 
  ...
  json_array_foreach(arr, index, node){
    if(json_is_object(node)){
      total += print_cf_account(node, level, currency_format, currency_symbol, fp);
    }
  }
  return total;
} 
...
bool process_cash_flow(const json_t *root, int from_date,int to_date, int date_format, 
  ...
  do{
    ...
    double org_beg_cash_balance = beg_cash_balance;
    ...
    double net_income = 0;
    double org_net_income = 0;
    cash_handle= json_object_get(report_handle, "net_income");
    if(cash_handle && json_is_number(cash_handle)){
      org_net_income = net_income = json_number_value(cash_handle);   
    }
    ...
    const char *net_income_str = "<tr class=\"mat-row hide-bottom-border \" ><td class=\"mat-cell  \"><span class=\"d-flex align-items-center\" style=\"margin-left: 25px;\"><span class=\"app-link \">Net Income</span></span></td><td class=\"mat-cell  \"><span class=\"font-weight-500\"></span></td><td class=\"mat-cell report_total \"><span class=\"app-link \">";
    fwrite(net_income_str, 1, strlen(net_income_str), fp);
    ...
    if(net_income){
      ...
      char* formatted_total_net_income = get_formatted_currency(currency_format, net_income);
      if(formatted_total_net_income){
        fwrite(formatted_total_net_income, 1, strlen(formatted_total_net_income), fp);
        free(formatted_total_net_income);
      }
    }
    fwrite("</span></td></tr>", 1, strlen("</span></td></tr>"), fp);
    ...
    total_op_activities += org_net_income;
    ...
    double org_total_op_activities = total_op_activities ;
    ...
    if(arr_handle && json_is_array(arr_handle)){
      ...
      total_investing_activities += print_cf_accnts_from_array(arr_handle, 1, 
              currency_format, currency_symbol, "Investing Activities", fp);
    }
    ...
    double org_total_investing_activities = total_investing_activities;
    ...
    double org_total_financing_activities = total_financing_activities;
    ...
    double org_net_change_cash = net_change_cash = org_total_op_activities + 
                    org_total_investing_activities + org_total_financing_activities;
    ...
    end_cash_balance= org_net_change_cash - org_beg_cash_balance;
